...
usage()
{
      echo "Usage: batch_localize [ -d | --db_descriptor db_descriptor ] [ -q | --query_descriptors query_descriptor ]
			...
			[ --extractor extractor ] [ --matcher matcher ] [ --feature_output feature_output] [ --matcher_output match output]
                        [ -s | --sfm_workspace sfm_workspace ] [ -n | --num_matched num_matched ]"
      exit 2
}

PARSED_ARGUMENTS=$(getopt -a -n batch_localize -o d:q:i:s:n:c:o: --long db_descriptor:,query_desciptor:,query_images:,sfm_workspace:,db_prefix:,query_prefix:,num_matched:,camera_intrinsics:,output:,extractor:,matcher:,feature_output:,matcher_output: -- "$@")
VALID_ARGUMENTS=$?
